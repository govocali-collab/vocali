...
interface FounderApplicationEmailParams {
  fullName: string
  company: string
  email: string
  phone: string
...
/** Notification interne : une demande via le formulaire « place fondatrice » de la démo. */
export async function sendFounderApplicationEmail(p: FounderApplicationEmailParams) {
  const year = new Date().getFullYear()
...
<table width="100%" cellpadding="0" cellspacing="0" style="background:#FAF7F2;padding:40px 0;"><tr><td align="center">
  <table width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#FFFFFF;border-radius:12px;border:1px solid #EDE3D4;">
    <tr><td style="padding:32px 48px 8px;">
      <p style="margin:0 0 4px;font-size:13px;color:#A88840;font-weight:600;text-transform:uppercase;letter-spacing:1px;">🔥 Demande — Offre fondatrice</p>
      <h1 style="margin:0;font-size:22px;color:#2A2A2E;">${p.company}</h1>
      <p style="margin:8px 0 0;font-size:14px;color:#8A8A8E;">Formulaire de qualification (page démo).</p>
    </td></tr>
    <tr><td style="padding:16px 48px;">
      <table width="100%" cellpadding="0" cellspacing="0">
        ...
        ${row("Type d'entreprise", p.businessType)}
        ${row("Appels / semaine", p.callVolume)}
        ${row("Manque des appels ?", p.missedCalls)}
        ${row("Souhaite régler", p.urgency)}
        ${row("Plus grand défi", p.challenge)}
      </table>
    </td></tr>
