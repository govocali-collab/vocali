...
export async function updateClinicConfig(
  ...
  const updatedConfig = {
    ...
    // Message d'accueil personnalisé (vide = message par défaut).
    ...(config.greeting !== undefined ? { greeting: config.greeting.trim() || null } : {}),
  }
...
export async function syncLocationFromClinic(clinicId: string): Promise<void> {
  ...
  const payload = {
    ...
    agent_name: (config.agent_name as string) ?? "Sarah",
