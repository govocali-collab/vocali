...
async function getInvoices(email: string) {
  ...
  const invoices = await stripe.invoices.list({ customer: customers.data[0].id, limit: 24, status: "paid" })
...
export default async function SettingsPage() {
  ...
  const invoices = clinic.owner_email ? await getInvoices(clinic.owner_email).catch(() => []) : []
  ...
  const tabs = [
    ...
    { key: "recus", label: "Reçus", content: <BillingTable invoices={invoices} /> },
  ]
