...
async function fetchStats() {
  ...
  const callsGrowth = callsLastMonthTotal > 0 ? ((callsThisMonth - callsLastMonthTotal) / callsLastMonthTotal) * 100 : null
  const activeClinicsRate = totalClinics ? Math.round(((activeClinics ?? 0) / totalClinics) * 100) : 0

  return {
    mrr, revenueThisMonth, revenueLastMonth, revenueGrowth, churnThisMonth, failedPayments,
    totalClinics: totalClinics ?? 0, activeClinics: activeClinics ?? 0, pausedClinics: pausedClinics ?? 0,
    activeClinicsRate, newClinicsThisMonth: newClinicsThisMonth ?? 0, newClinicsLastMonth: newClinicsLastMonth ?? 0,
    callsThisMonth, callsGrowth, leadsThisMonth,
  }
}
...
export default async function MainStats() {
  ...
  return (
    <div>
      <p className="text-charcoal-500 text-sm mb-6 capitalize">{monthLabel}</p>

      <Section title="Revenus">
        <MetricCard label="MRR" value={formatCAD(s.mrr)} sub="revenu mensuel récurrent" accent />
        <MetricCard
          label={`Revenus — ${new Date().toLocaleDateString("fr-CA", { month: "short" })}`}
          value={formatCAD(s.revenueThisMonth)}
          sub={s.revenueGrowth !== null ? `${formatPct(s.revenueGrowth)} vs mois dernier (${formatCAD(s.revenueLastMonth)})` : `Mois dernier : ${formatCAD(s.revenueLastMonth)}`}
          trend={s.revenueGrowth}
        />
        <MetricCard label="Paiements échoués" value={String(s.failedPayments)} sub="ce mois" alert={s.failedPayments > 0} />
        <MetricCard label="Désabonnements" value={String(s.churnThisMonth)} sub="ce mois" alert={s.churnThisMonth > 0} />
      </Section>

      <Section title="Cliniques">
        <MetricCard label="Total" value={String(s.totalClinics)} sub="cliniques inscrites" />
        <MetricCard label="Actives" value={String(s.activeClinics)} sub={`${s.activeClinicsRate}% du total`} positive />
        <MetricCard label="En pause" value={String(s.pausedClinics)} sub="à suivre" alert={s.pausedClinics > 0} />
        <MetricCard
          label="Nouvelles ce mois" value={String(s.newClinicsThisMonth)} sub={`${s.newClinicsLastMonth} le mois dernier`}
          trend={s.newClinicsLastMonth > 0 ? ((s.newClinicsThisMonth - s.newClinicsLastMonth) / s.newClinicsLastMonth) * 100 : null}
        />
      </Section>

      <Section title="Activité agent">
        <MetricCard label="Appels ce mois" value={String(s.callsThisMonth)} sub={s.callsGrowth !== null ? `${formatPct(s.callsGrowth)} vs mois dernier` : "ce mois"} trend={s.callsGrowth} />
        <MetricCard label="Leads capturés" value={String(s.leadsThisMonth)} sub="ce mois" positive={s.leadsThisMonth > 0} />
        <MetricCard label="Taux de conversion" value={s.callsThisMonth > 0 ? `${Math.round((s.leadsThisMonth / s.callsThisMonth) * 100)}%` : "—"} sub="leads / appels" />
      </Section>
    </div>
  )
}
...
function MetricCard({
  ...
  label: string; value: string; sub?: string; accent?: boolean; positive?: boolean; alert?: boolean; trend?: number | null
}) {
  const trendPositive = trend !== null && trend !== undefined && trend > 0
  const trendNegative = trend !== null && trend !== undefined && trend < 0
  return (
    <div className={cn("bg-white rounded-xl border p-4 shadow-card", accent ? "border-gold-300 bg-gold-50/30" : "border-ivory-300", alert ? "border-red-200 bg-red-50/20" : "")}>
      <p className="text-charcoal-400 text-[11px] font-body uppercase tracking-wider mb-2">{label}</p>
      <p className={cn("text-2xl font-display font-semibold mb-1", accent ? "text-gold-700" : positive ? "text-green-700" : alert ? "text-red-600" : "text-charcoal-900")}>{value}</p>
      {sub && <p className={cn("text-xs font-body", trendPositive ? "text-green-600" : trendNegative ? "text-red-500" : "text-charcoal-400")}>{sub}</p>}
    </div>
  )
}
